...
necessary preconditions are (known as backwards
chaining\cite{Russell05}). If both the precondition and postcondition
of some sequence of code are known, then the proof can progress
inwards from both ends, meeting in the middle. If the two proof
efforts cannot be unified, then there is a flaw in the
code.
...
hold. Similarly, postcondition $Q$ implies $Q'$, then by using the
postcondition $Q'$, we know that $Q$ is guaranteed to hold. This is
...
\subsubsection{Array Reasoning}
\label{sec:lit-verification-embedding-arrays}
...
an array of type $[T]$ is treated as a function $(N \subseteq \mathbb
N) \to T$, and array assignment is modelled as functional override.

The formalism has preconditions and postconditions for array access to
ensure indices are in bounds, and by modelling assignment as override,
updating an array in one location cannot cause it to appear to change
in another (the problem of aliasing), which simplifies reasoning.
...
proof method. Proof assistants may be more useful here than in
...
refinement, a different copying collector verified by embedding, and
...
generic verification of collectors. In future chapters I shall be
