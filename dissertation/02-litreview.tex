...
allocated memory from a program\todo{cite}. In languages with manual
...
and the program may gradually consume more and more memory\todo{cite}.
...
mark-compact\todo{cite}. Each is suited to different use-cases, and the
...
modified, to offset this\todo{cite}. However, this places a burden on
...
marked are unmarked, ready for the next collection\todo{cite}.
...
becomes more frequent as heap residency increases\todo{cite}, and the
blocking time is proportional to the size of the heap\todo{cite}.
...
heap\todo{cite}). Hughes's lazy sweep algorithm\cite{Hughes82} does
...
proportional only to the number of live cells\todo{cite}.
...
Heap fragmentation can be a great problem\todo{cite}: there may be more
...
fragmentation\todo{cite}. There are three ways of performing this
...
generation\todo{cite}. In the simplest case, all live cells at the
...
where old-to-young pointers are rare\todo{cite}, such as most
functional languages, and so has become fairly popular amongst
implementers of such languages\todo{cite}.
