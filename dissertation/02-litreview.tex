...
\begin{align*}
  I[1/j][0/i] &\equiv xs[0] = \min \{xs[a]~|~0 \leq a < 1\}\\
  &\equiv \len xs > 0 \implies xs[0] = \min \{xs[0]\}\\
  &\equiv \len xs > 0 \implies xs[0] = xs[0]\\
  &\equiv \len xs > 0 \implies \true\\
  &\equiv \len xs > 0\\
  \therefore I[1/j][0/i] &\iff L\\\\
%
  S[j/i] &\equiv \left(I \land xs[i] \leq xs[j] \land R(j)\right)[j/i]\\
  &\equiv I \land xs[j] \leq xs[j] \land R(j)\\
  &\equiv I \land R(j)\\
  \therefore C_{T} &\implies S[j/i]\\\\
%
  I[j+1/j] &\equiv I \land xs[i] \leq xs[j]\\
  \therefore S &\implies I[j+1/j]
\end{align*}

\begin{align*}
  \lnot\left(xs[j] < xs[i]\right) &\equiv xs[i] \leq xs[j]\\
  \therefore C_{F} &\iff S\\\\
%
  I \land xs[i] \leq xs[j] \land R(j) &\implies I\\
  \therefore S &\implies I\\\\
%
  \lnot R(j) \land I &\equiv j \geq \len xs \land xs[i] = \min
  \{xs[a]~|~0 \leq a < j\}\\
  &\equiv xs[i] = \min \{x~|~x \in xs\}\\
  &\equiv xs[i] = \min xs\\
