...
\begin{definition}[Cell]
  A cell is a contiguous sequence of one or more contiguous words in
  the heap, one of which is the garbage collection metadata
  word. Metadata words precede data words, forming a header.
\end{definition}
...
\glspl{cell} which point to each other (and is really the only way
that fragmentation could even be implemented). I have reserved an
...
\todo{Everything pointed to gets marked}
